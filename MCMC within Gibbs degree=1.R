##------------------------------------------------------------------------------
##---------------------------------MCMC within Gibbs ALL -----------------------
###-------------------------simulation d=1  without integral -------------------
##-------------------------- sample size 50----------------------------------------
##------------------------------------------------------------------------------
...
#----------------------posterior def without integral ---------------------
...
post_beta1_d1=Vectorize(function(be1){
  ...
  IsR=c()
  for(i in 1:n){ 
    IsL[i]=sum(gamma*iSpline(LR[i,1], knots = knots[-c(1,22)], degree = 0, Boundary.knots=c(0,14)))
    IsR[i]=sum(gamma*iSpline(LR[i,2], knots = knots[-c(1,22)], degree = 0, Boundary.knots=c(0,14)))
  }
  final=c()
  for(i in 1:n){
    DEN=c(0)
    for (l in 1:(interior_knot+1)) {
      lp=sum(gamma[1:(l-1)])
      DEN[l]=exp((-exp(be1*Z[i,1]+be2*Z[i,2])*lp)+
                   ((s[l]*gamma[l]*exp(be1*Z[i,1]+be2*Z[i,2])) /(s[l+1]-s[l])))*
        (   ((s[l+1]-s[l])/(-gamma[l]*exp(be1*Z[i,1]+be2*Z[i,2])))*
              ( exp((-gamma[l]*s[l+1]*exp(be1*Z[i,1]+be2*Z[i,2]))/(s[l+1]-s[l])) - 
                  exp((-s[l]*gamma[l]*exp(be1*Z[i,1]+be2*Z[i,2]))/(s[l+1]-s[l]))   )  )
    }
...
post_gamma_d1=Vectorize(function(dxp,g1,g2,g3,g4,g5,g6,g7,g8,g9,g10,g11,g12,g13,g14,g15,g16,g17,
  ...
  gamma=c(g1,g2,g3,g4,g5,g6,g7,g8,g9,g10,g11,g12,g13,g14,g15,g16,g17,
          g18,g19,g20,g21)
  for(i in 1:n){ 
    Isgamma_L[i]=sum(gamma*iSpline(LR[i,1], knots = knots[-c(1,22)], degree = 0, Boundary.knots=c(0,14)))
    Isgamma_R[i]=sum(gamma*iSpline(LR[i,2], knots = knots[-c(1,22)], degree = 0, Boundary.knots=c(0,14)))
  }
  final=c()
  for(i in 1:n){
    DEN=c(0)
...
be2=0
xbeta1=c()
xbeta1[1] <- rnorm(1,0,1)
xbeta2=c()
xbeta2[1] <- rnorm(1,0,1)
xgamma=matrix(0,4000,21)
for (i in 1:21) {
...
for (i in 2:4000) {
  ...
  xt <- xbeta1[i-1]
  y <- rnorm(1,xt,1)
  num <- post_beta1_d1(y) * dnorm(xt,y,1)
  den <- post_beta1_d1(xt) * dnorm(y,xt,1)
  if (u <= num/den) {
    xbeta1[i] <- y 
  }
  else {
  ...
  xt <- xbeta2[i-1]
  y <- rnorm(1,xt,1)
  num <- post_beta2_d1(y) * dnorm(xt,y,1)
  den <- post_beta2_d1(xt) * dnorm(y,xt,1)
  ...
  #lamda_hat=mean(rgamma(100000,20,sum(gamma)+1))
}
